import sability from "./AyraStarrSability.mp3";
import ReasonYou from "./ReasonYou.mp3";
import remaHoliday from "./RemaHoliday.mp3";
import Soso from "./soso.mp3";
import WonDaMo from "./WonDaMo.mp3";
import RemaCalmDown from "./RemaCalmDown.mp3";
import EllP from "./Libianca-People-(TrendyBeatz.com).mp3";
import yoga from "./Asake-Yoga-New-Song-(TrendyBeatz.com).mp3";
import abgracadabra from "./Rexxie-Ft-Skiibii-and-Naira-Marley-Abracadabra-(TrendyBeatz.com).mp3";
import calmdown from "./Rema-Ft-Selena-Gomez-Baby-Calm-Down-Remix-(TrendyBeatz.com).mp3";
import asiwaju from "./Ruger-Asiwaju-New-Song-(TrendyBeatz.com).mp3";
import apollo2 from "./Victony-Apollo-(TrendyBeatz.com).mp3";
import jolene from "./Victony-Ft-Ktizo-Jolene-(TrendyBeatz.com).mp3";
import soweto from "./Victony-Ft-Tempoe-Soweto-(TrendyBeatz.com).mp3";
import kolomentall from "./Victony-Kolomental-(TrendyBeatz.com).mp3";
import folake2 from "./Boy-Spyce-Folake-New-Song-(TrendyBeatz.com).mp3";
import rush from "./Ayra-Starr-Rush-(TrendyBeatz.com).mp3";
import myguy from "./Tiwa-Savage-Ft-Spyro-Who-Is-Your-Guy-Remix-(TrendyBeatz.com).mp3";
import joha from "./Asake-Joha-New-Song-(TrendyBeatz.com).mp3";
import jol from "./Khaid-Jolie-(TrendyBeatz.com).mp3"

export {
  ReasonYou,
  RemaCalmDown,
  WonDaMo,
  remaHoliday,
  sability,
  Soso,
  EllP,
  abgracadabra,
  apollo2,
  asiwaju,
  calmdown,
  folake2,
  joha,
  jolene,
  kolomentall,
  myguy,
  rush,
  soweto,
  yoga,
  jol
};
